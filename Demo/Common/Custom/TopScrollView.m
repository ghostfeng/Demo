...
static CGFloat kDistanceBetweenItem = 10;
...
- (void)makeItemWithTitleIndex:(NSInteger)index
{
    ...
    [item setTitleColor:[UIColor lightGrayColor] forState:UIControlStateSelected];
    [item addTarget:self
...
- (void)itemClick:(UIButton *)sender {
    if (self.selectedBtn != sender) {
        ...
        if (self.listItemClickBlock) {
            self.listItemClickBlock(sender.titleLabel.text,[self findIndexOfListsWithTitle:sender.titleLabel.text]);
        }
    }
...
-(NSInteger)findIndexOfListsWithTitle:(NSString *)title{
    for (NSInteger i =0; i < self.dataList.count; i++) {
        if ([title isEqualToString:self.dataList[i]]) {
            return i;
        }
    }
    return 0;
